...
class asymmetrical_SimSiamRecognizer2D(Recognizer2D):
    def __init__(self,
                 backbone,
                 cls_head=None,
                 projectionMLP=None,
    ...
    def forward_train(self, imgs_pathway_A, imgs_pathway_B, labels, **kwargs):
        ...
        x_pathway_A = x_pathway_A.squeeze_()
        x_pathway_B = x_pathway_B.squeeze_()
        # x_pathway_A = torch.flatten(x_pathway_A, 1)
        # x_pathway_B = torch.flatten(x_pathway_B, 1)

        #in the projection MLP we are going to perform Dim reduction or average pooling
        ...
        # if torch.allclose(contrastive_pathway_A_features[0][0],contrastive_pathway_B_features[0][0]):
        #     assert print('These two features should not be same unless both path way have the same input')
        #in color augmentation some video are not augmented 

        features = x_pathway_A.detach()
        cls_score_pathway_A = self.cls_head(features.float(), num_segs)
        ...
        loss_self_supervised = self.color_contrastive_loss(predict_features_A_features,
                                        contrastive_pathway_B_features.detach(), label=gt_labels)
...
@RECOGNIZERS.register_module()
class symmetrical_SimSiamRecognizer2D(Recognizer2D):
    def __init__(self,
                 backbone,
                 cls_head=None,
                 projectionMLP=None,
    ...
    def forward_train(self, imgs_pathway_A, imgs_pathway_B, labels, **kwargs):
        ...
        contrastive_pathway_A_features = self.projectionMLP(x_pathway_A.float())
        contrastive_pathway_B_features = self.projectionMLP(x_pathway_B.float())
        if contrastive_pathway_A_features[0]==contrastive_pathway_B_features[0]:
            assert print('These two features should not be same unless both path way have the same input')
        ...
        loss_cls = self.cls_head.loss(cls_score_pathway_A, gt_labels, **kwargs)
        #predictMLP for both feature_A and feature_B
        predict_features_A_features = self.predictionMLP(contrastive_pathway_A_features)
        predict_features_B_features = self.predictionMLP(contrastive_pathway_B_features)

        q1, k1, q2, k2 = predict_features_A_features, contrastive_pathway_B_features.detach(), predict_features_B_features, contrastive_pathway_A_features.detach()
        ...
        if self.type_loss == 'contrastive':
            loss_self_supervised = self.color_contrastive_loss(q1, k1, q2, k2)
        else:
            loss_self_supervised = self.color_contrastive_loss(q1, k1, q2, k2, gt_labels)
