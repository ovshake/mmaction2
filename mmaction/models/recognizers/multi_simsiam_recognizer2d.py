...
class Mult_SimSiam_Recognizer2D(Recognizer2D):
    '''using multiple simsiam heads to train the model'''
    def __init__(self,
                 backbone,
                 cls_head=None,
                 projectionMLP=None,
        ...
        self.projectionMLP_heads = []
      

        self.predictionMLP = builder.build_head(predictionMLP)
        for _ in range(num_heads):
            self.projectionMLP_heads.append(builder.build_head(projectionMLP).to(device))
    ...
    def forward_train(self, imgs_pathways, labels, **kwargs):
        ...
        cls_scores = self.cls_head(processed_pathways[0].float(), -1)
        loss_cls = self.cls_head.loss(cls_scores, labels, **kwargs)
        losses.update(loss_cls)
        for idx in range(len(self.projectionMLP_heads)):
            # print(idx)
            # print(len(self.projectionMLP_heads))
            if idx ==0:
                h_img = self.projectionMLP_heads[idx](processed_pathways[idx].float())
                h_img = self.predictionMLP(h_img)
