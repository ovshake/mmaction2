...
class ToTensor(object):
    ...
    def __call__(self, sample):
        images, labels = sample['imgs'], sample['label']
        images = images.transpose((0, 3, 1, 2))
        return {'imgs': torch.from_numpy(images).float().div(255.),
...
class Normalize(object):
    ...
    def __call__(self, sample):
        ...
        return {'imgs': images, 'label': labels}



@DATASETS.register_module()
class GolfDB(Dataset):
    def __init__(self, data_file, vid_dir, seq_length, train=True):
        self.df = pd.read_pickle(data_file)
        self.vid_dir = vid_dir
        self.seq_length = seq_length
        norm = Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])  # ImageNet mean and std (RGB)
        self.transform = transforms.Compose([ToTensor(), norm])
        self.train = train
    ...
    def __getitem__(self, idx):
        ...
        sample = {'imgs':np.asarray(images), 'label':np.asarray(labels)}
        if self.transform:
