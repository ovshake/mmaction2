...
@DATASETS.register_module()
class EpicKitchensTemporalSpatialMMSADA_ensemble(BaseDataset):
    def __init__(self,
                 domain,
                 pathway_A,
                 clip_len,
    ...
    def prepare_train_frames(self, idx):
        ...
        pathway_A_start_index = np.random.randint(start_index, max(end_index - num_frames, start_index + 1))
        # pathway_B_start_index=pathway_A_start_index
        # pathway_C_start_index=pathway_A_start_index
        # if self.not_same_start_frame:
       ...
       #both pathways has to have the same start frame or idx
        ...
        return self.pathway_A(pathway_A_results), self.pathway_A(pathway_A_results),self.pathway_A(pathway_A_results)  # teh sencond on is pathway_B third one is pathway_C
