...
model = dict(
             type='L2DRecognizer2D',
            backbone=dict(type='ResNetTSM',
                depth=50,
                norm_eval=False,
                norm_cfg=dict(type='SyncBN', requires_grad=True),
                shift_div=8),
            cls_head=dict(num_segments=8,dropout_ratio=0.0,in_channels=2048, num_classes=8),
            contrastive_loss=dict(type='SupConLoss'))
...
# optimizer
optimizer = dict(type='SGD',constructor='TSMOptimizerConstructor',nesterov=True, momentum=0.9, weight_decay=0.0005,
    lr=0.0075 * (12 / 8) * (4 / 8),  # this lr is used for 8 gpus in the paper learning rate is o.oo1  learning rate for augnet is 0.005
)
