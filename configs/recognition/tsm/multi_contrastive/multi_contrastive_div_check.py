...
model = dict(
            ...
            cls_head=dict(num_segments=clip_len,
                        ...
                        in_channels=2048,
                        dropout_ratio=0.0),
            num_heads=4,#MultiplePathwayBaselineContrastiveLoss    #Multi_Contrastive_Loss_each_space
            contrastive_loss=dict(type='MultiplePathwayBaselineContrastiveLoss_div',
            use_row_sum_a=True,
                                    use_row_sum_b=True,
                                    use_positives_in_denominator=True),
            contrastive_head=dict(type='projection_MLP_multi',
                                num_segments=8,
                                feature_size=2048),
...
optimizer = dict(
    ...
    lr=0.0075 * (10 / 8) * (1 / 8),  # this lr is used for 8 gpus
    momentum=0.9,
    weight_decay=0.0001)
