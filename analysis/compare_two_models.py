...
def parse_args():
    ...
    parser.add_argument('-t', '--test-domain', required=True, type=str)
...
def get_verbclass_to_verb_mapping(data_labels):
    ...
    for data in data_labels:
        mapping[data['class']] = data['label'] 
    return mapping 

 
def load_annotations(domain_csv_file_path):
    df = pd.read_csv(domain_csv_file_path) 
    data_labels = []
    count = 0
    for _, line in df.iterrows():
        ...
        _class = line['class'] 
        _class = int(_class)
        metadata_dict = {"index": count, "path": path, "class": _class, "label": label, 'start_frame': start_frame, 'stop_frame': end_frame}
...
def join_frames_to_video(data, domain, write_dir=None, pred_cls=None, gt_cls=None, args=None):
    ...
    path = data['path']
    video_id = '_'.join(path.split('/')[-2:])

    frame_fmt = 'img_{:05d}.jpg' 
    frame_path_fmt = f'{path}/{frame_fmt}'
...
def pair_videos_with_gradcam(write_dir):
    video_gifs = glob(join(write_dir, 'video_*.gif')) 
    gradcam_gifs = glob(join(write_dir, 'gradcam_*.gif'))
    videos = dict() 
    for video_gif in video_gifs:
        video_name = video_gif.split('/')[-1].split('.')[0] 
        video_name = video_name.replace("video_", "") 
        videos[video_name] = [video_gif] 
    
    for gradcam_gif in gradcam_gifs:
        video_name = gradcam_gif.split('/')[-1].split('.')[0]
        video_name = video_name.replace("gradcam_", "") 
        videos[video_name].append(gradcam_gif) 
...
if __name__ == '__main__':
    ...
    if args.verbose:
        for idx, index in enumerate(diff_in_preds):
            ...
            missclassified_classes.append(test_labels[index]['label']) 
    ...
    if args.wandb:
        ...
        video_gradcam_pairing = pair_videos_with_gradcam(args.write_dir) 

        for item in video_gradcam_pairing.values(): # zip(video_gifs, gradcam_gifs)
            if len(item) != 2:
                continue 
            video_gif, gradcam_gif = item 
        ...
        all_gt = [k['class'] for k in test_labels]
        wandb.log({"conf_mat" : wandb.plot.confusion_matrix(probs=None,
                        y_true=all_gt, preds=all_preds,
                        class_names=[verbclass_2_verb_mapping[k] for k in range(38)])}) 
